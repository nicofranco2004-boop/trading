...
--
-- DIALECTO
--   Escrito para PostgreSQL. Los tres puntos donde SQLite difiere están
--   marcados con  -- [SQLITE]  y la variante al lado.
--
...
SELECT
    ...
    ROUND(SUM(COALESCE(p.invested,0) + COALESCE(p.commissions,0))::numeric, 2)
...
SELECT
    SUM(CASE WHEN parent_es_ars THEN 1 ELSE 0 END)          AS a1_subbroker_usd_de_padre_ars,
    SUM(CASE WHEN NOT parent_es_ars THEN 1 ELSE 0 END)      AS a2_broker_usd_genuino,
    COUNT(DISTINCT CASE WHEN NOT parent_es_ars THEN user_id END)
                                                            AS a2_usuarios
FROM (
    SELECT
        p.user_id,
...
SELECT
    ...
    ROUND(SUM(ABS(capital_inicio))::numeric, 2)        AS monto_total_declarado
FROM (
    SELECT
...
SELECT
    ...
    COUNT(DISTINCT user_id)                         AS usuarios,
    ROUND(SUM(COALESCE(cost_basis_consumed,0))::numeric, 2)
                                                    AS sobreestimacion_total_pnl,
    ROUND(SUM(COALESCE(pnl_usd,0))::numeric, 2)     AS pnl_publicado_total,
...
SELECT
    ...
    COUNT(DISTINCT user_id)                     AS usuarios,
    ROUND(AVG(ratio)::numeric, 4)               AS ratio_promedio,
    ROUND(SUM(manual_deposits)::numeric, 2)     AS usd_registrados_con_ese_tc
FROM (
...
SELECT
    ...
    COUNT(DISTINCT user_id)                     AS usuarios,
    ROUND(SUM(ABS(COALESCE(pnl_usd,0)))::numeric, 2) AS pnl_nominal_afectado,
    MIN(date) AS primera, MAX(date) AS ultima
FROM operations
...
SELECT
    ...
    ROUND(SUM(fantasma)::numeric, 2)                AS fantasma_neto_usd,
    ROUND(MAX(ABS(fantasma))::numeric, 2)           AS peor_caso_usd
FROM (
    SELECT
        t.batch_id,
...
SELECT
    ...
    ROUND(SUM(CASE WHEN UPPER(COALESCE(currency,'')) NOT IN ('USD','USDT')
                   ...
                   THEN COALESCE(pnl_usd,0) ELSE 0 END)::numeric, 2)
