export const meta = {
  name: 'auditoria-rendi-origin-main',
  description: 'Auditoría técnica externa read-only de Rendi sobre origin/main (producción): mapa de sistema, modelo de datos, endpoints, pantallas, conceptos de negocio, jobs, servicios externos y código muerto',
  phases: [
    { title: 'Leer', detail: 'lectores paralelos por subsistema (backend, frontend, infra)' },
    { title: 'Conceptos', detail: 'un agente por concepto de negocio, rastreo exhaustivo con file:line' },
    { title: 'Verificar', detail: 'verificación adversarial de citas archivo:línea y afirmaciones' },
    { title: 'Reparar', detail: 'corrección de lo que no resistió la verificación' },
    { title: 'Criticar', detail: 'críticos de completitud con 4 lentes distintas' },
  ],
}
...
// ── EJECUCIÓN ───────────────────────────────────────────────
phase('Leer')
log(`Auditando origin/main (b74f450f). ${READERS.length} lectores en paralelo sobre 1.126 archivos.`)

const lectores = (await parallel(READERS.map(r => () =>
  agent(`${RULES}\n\n---\n\n# TU TAREA: ${r.title}\n\n${r.prompt}\n\nCuando termines de escribir el archivo, devolvé el objeto estructurado.`,
    { label: r.label, phase: 'Leer', schema: READER_SCHEMA })
))).filter(Boolean)

log(`${lectores.length}/${READERS.length} lectores listos. Ahora ${CONCEPTOS.length} conceptos de negocio.`)

phase('Conceptos')

const conceptos = (await parallel(CONCEPTOS.map(([id, slug, nombre, pistas]) => () =>
  agent(`${RULES}
...
Escribí en \`${PARTS}/30-concepto-${slug}.md\` y devolvé el objeto estructurado.`,
    { label: `concepto:${slug}`, phase: 'Conceptos', schema: CONCEPT_SCHEMA })
))).filter(Boolean)

log(`${conceptos.length}/${CONCEPTOS.length} conceptos rastreados. Verificación adversarial de citas…`)
...
const VERIFY_SCHEMA = {
  ...
  properties: {
    ...
    afirmacionesFalsas: { type: 'array', items: { type: 'string' } },
    necesitaReparacion: { type: 'boolean' },
  },
  required: ['file', 'revisadas', 'malas', 'detalles', 'afirmacionesFalsas', 'necesitaReparacion'],
}

const archivos = [...lectores.map(r => r.file), ...conceptos.map(c => c.file)].filter(Boolean)
...
# TU TAREA: verificación adversarial

Otro auditor escribió \`${f}\`. Tu trabajo es **desconfiar de él**.

1. Leelo entero.
2. Extraé todas las citas \`archivo:línea\`. Si hay más de 70, tomá una muestra de 70 priorizando las que sostienen las afirmaciones más fuertes (fórmulas, permisos, esquema de datos, model ids, umbrales).
3. Para cada cita, **abrí la línea de verdad**: \`cd ${TREE} && sed -n 'N-4,N+4p' <ruta>\`. Comprobá que ahí esté lo que el documento afirma. Tolerancia: ±4 líneas está bien (marcala como buena). Un archivo equivocado, una función distinta o una línea que no tiene nada que ver, NO.
4. Elegí las **10 afirmaciones más sustantivas** del documento (las que un founder usaría para decidir algo) e intentá **refutarlas** leyendo el código. Tu sesgo por defecto: la afirmación es falsa hasta que la veas.
5. Reportá cada cita mala como "lo que dice el doc → lo que hay realmente ahí", y cada afirmación que el código contradice.

**No modifiques el archivo.** Solo reportá. Devolvé el objeto con \`file\` = "${f}".`,
    { label: `verificar:${f.split('/').pop()}`, phase: 'Verificar', schema: VERIFY_SCHEMA, effort: 'high' }),

  (v, f) => {
    if (!v || !v.necesitaReparacion || (v.malas === 0 && (v.afirmacionesFalsas || []).length === 0)) {
      return { file: f, reparado: false, arreglos: 0, malas: v ? v.malas : 0 }
    }
    return agent(`${RULES}

---

# TU TAREA: reparar un documento de auditoría

\`${f}\` tiene errores detectados por un verificador. Corregilos **en ese mismo archivo** (está en el scratchpad, se puede editar).

Citas incorrectas:
${(v.detalles || []).map(d => `- ${d}`).join('\n') || '(ninguna)'}

Afirmaciones que el código contradice:
${(v.afirmacionesFalsas || []).map(d => `- ${d}`).join('\n') || '(ninguna)'}

Para cada una:
1. Volvé al código en \`${TREE}\` y buscá dónde está REALMENTE lo que se quería citar (\`grep -n\`).
2. Si lo encontrás, corregí el número de línea.
3. Si NO existe, reescribí la afirmación con **no encontrado** o borrala. Nunca dejes una cita inventada.
4. Si la afirmación era falsa, reescribila con lo que el código realmente hace.

Ediciones quirúrgicas: no reescribas el documento entero, conservá todo lo que estaba bien.`,
      { label: `reparar:${f.split('/').pop()}`, phase: 'Reparar', effort: 'high',
        schema: { type: 'object', properties: { file: { type: 'string' }, reparado: { type: 'boolean' }, arreglos: { type: 'integer' }, notas: { type: 'string' } }, required: ['file', 'reparado', 'arreglos'] } })
  }
)

const totalArreglos = verificados.filter(Boolean).reduce((s, v) => s + (v.arreglos || 0), 0)
log(`Verificación completa (${totalArreglos} correcciones). Críticos de completitud…`)
...
const CRITIC_SCHEMA = {
  ...
  properties: {
    ...
    hallazgosClave: { type: 'array', items: { type: 'string' } },
    preguntasFounder: { type: 'array', items: { type: 'string' } },
  },
  required: ['huecos', 'contradicciones', 'hallazgosClave', 'preguntasFounder'],
}

const LENTES = [
  ['cobertura', `Recorré el repo en \`${TREE}\` y comprobá qué quedó SIN documentar. Listá TODOS los archivos de más de 300 líneas cuyo nombre no aparece en ningún .md de ${PARTS} (script sugerido: por cada archivo grande, \`grep -l "nombre.py" ${PARTS}/*.md\`). También carpetas o subsistemas enteros ausentes.`],
  ['coherencia', `Cruzá las partes entre sí y buscá CONTRADICCIONES: dos partes que digan cosas incompatibles sobre la misma tabla, el mismo endpoint o el mismo cálculo. Cuando encuentres una, andá al código y resolvé cuál tiene razón — o determiná si el sistema realmente tiene dos comportamientos distintos (que es el hallazgo más interesante).`],
  ['riesgo', `Leé todo con ojo de RIESGO. Dónde puede el sistema (a) mostrarle al usuario un número financiero incorrecto, (b) dejar que alguien vea datos de otro usuario, (c) perder datos o plata, (d) cobrar de más o de menos. No propongas fixes: nombrá el riesgo, citá el código, y decí cuál sería el síntoma visible.`],
  ['negocio', `Leé todo con ojo de NEGOCIO y de founder. ¿Qué conceptos de negocio quedaron sin rastrear? ¿Qué features existen en el código que nadie documentó? ¿Qué decisiones de producto están enterradas en el código como constantes mágicas o umbrales? ¿Qué necesita responder el founder para que esta auditoría sirva?`],
]

const criticos = (await parallel(LENTES.map(([lente, foco]) => () =>
  agent(`${RULES}

---

# TU TAREA: crítico de completitud — lente «${lente}»

Ya se escribió un informe repartido en varios markdown en \`${PARTS}/\`. Listalos (\`ls ${PARTS}/\`) y leé los que necesites (si son muchos, navegá con grep).

${foco}

Tu salida NO es un resumen del informe: es la lista de lo que FALTA o está MAL. Concreto y accionable — "falta documentar X que vive en archivo:línea", no "podría profundizarse".

Devolvé el objeto estructurado.`,
    { label: `crítico:${lente}`, phase: 'Criticar', schema: CRITIC_SCHEMA, effort: 'high' })
))).filter(Boolean)
...
return {
  ...
  correcciones: totalArreglos,
  hallazgos: lectores.flatMap(r => r.hallazgos || []),
  unknowns: [...lectores, ...conceptos].flatMap(r => r.unknowns || []),
  preguntas: [...lectores, ...conceptos].flatMap(r => r.questions || []),
  divergencias: conceptos.flatMap(c => c.implementacionesDivergentes || []),
  deadCode: lectores.flatMap(r => r.deadCode || []),
  huecos: criticos.flatMap(c => c.huecos || []),
  contradicciones: criticos.flatMap(c => c.contradicciones || []),
  hallazgosClave: criticos.flatMap(c => c.hallazgosClave || []),
  preguntasFounder: criticos.flatMap(c => c.preguntasFounder || []),
}
