export const meta = {
  name: 'auditoria-rendi-tanda-b',
  description: 'Inventario de los 8 conceptos de negocio que sostienen los números: costo/FIFO, P&L realizado, valuación, rendimiento, TIR, FX, capital aportado y caja',
  phases: [
    { title: 'Conceptos', detail: 'un agente por concepto, rastreando cada sitio de cálculo y lectura con archivo:línea' },
  ],
}
...
const ELEGIDOS = new Set([
  'costo', 'pnl-realizado', 'valuacion', 'rendimiento',
  'tir', 'fx', 'capital-aportado', 'caja',
])
const TANDA = CONCEPTOS.filter(([id, slug]) => ELEGIDOS.has(slug))

phase('Conceptos')
log(`Tanda B — ${TANDA.length} conceptos: ${TANDA.map(c => c[1]).join(', ')}`)
