export const meta = {
  ...
  phases: [
    { title: 'Leer', detail: '16 lectores pendientes: parsers, IA, billing, asesor, análisis, precios, jobs, externos, frontend, tests, código muerto, flujos' },
      ],
}
...
// ── EJECUCIÓN — TANDA A: sólo los lectores pendientes ───────
const PENDIENTES = new Set([
  '09-importing-b', '10-ai', '11-billing', '12-asesor', '13-analitica',
  '14-precios-fx', '15-jobs', '16-externos',
  '17-front-cartera', '18-front-analisis', '19-front-ops',
  '20-front-cuenta', '21-front-nucleo', '22-tests', '23-muerto', '24-flujos',
])
const TANDA = READERS.filter(r => PENDIENTES.has(r.id))

phase('Leer')
log(`Tanda A — ${TANDA.length} lectores pendientes sobre origin/main (b74f450f). Sin fase de verificación.`)

const lectores = (await parallel(TANDA.map(r => () =>
  agent(`${RULES}
...
## IMPORTANTE — escribí a disco antes de terminar
...
Cuando termines, devolvé el objeto estructurado.`,
...
return {
  ...
  completados: lectores.length,
  partes: lectores.map(r => r.file),
  hallazgos: lectores.flatMap(r => r.hallazgos || []),
  deadCode: lectores.flatMap(r => r.deadCode || []),
  unknowns: lectores.flatMap(r => r.unknowns || []),
  preguntas: lectores.flatMap(r => r.questions || []),
}
