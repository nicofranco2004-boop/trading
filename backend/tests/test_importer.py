...
class SplitRatioAdjustTest(unittest.TestCase):
    ...
    # La 2ª pasada por PRECIO se desactivó tras el audit (2026-08-10): el ratio
    # de una caída real de F veces daba exactamente 1.0, el centro de la banda,
    # así que la señal disparaba justo en el caso que creía excluir y el ajuste
    # multiplicaba la tenencia. Ver el comentario de _SPLIT_PRICE_EVIDENCE_ENABLED.
    def test_sin_fecha_de_compra_no_se_ofrece_por_precio(self):
        """Un lote cuya fecha es posterior al split NO se ajusta, aunque el
        precio 'parezca' pre-split: no hay forma de distinguirlo de una caída
        real y equivocarse acá triplica la tenencia del usuario."""
        conn = main.get_db()
        cur = conn.execute(
            """INSERT INTO positions (user_id, broker, asset, is_cash, buy_price, quantity,
                   invested, entry_date, asset_type)
        ...
        with patch.object(main, "_fetch_ba_splits", return_value=list(SPY_SPLITS)):
            res = self.client.get("/api/positions/split-check",
                                  headers={"Authorization": f"Bearer {self.token}"})
        self.assertNotIn(pid, {s["pid"] for s in res.json()["suggestions"]})
        # Y el endpoint de ajuste tampoco lo toca si lo llaman a mano.
        self._adjust(pid)
        self.assertAlmostEqual(self._row(pid)["quantity"], 10.0)
