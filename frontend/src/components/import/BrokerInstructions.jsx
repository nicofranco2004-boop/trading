...
const BROKERS = [
  ...
  },
  {
    ...
    steps: [
      ...
      '¿Tenés muchos años de historia y el portal no te deja exportar todo? Pedile a Bull Market el histórico de cuenta corriente por mail: te mandan uno o varios CSV con TODO tu historial (a veces uno solo tipo HC12345.CSV, a veces partido en un archivo por período, con nombres largos de números). Cualquiera de esas versiones sirve — subilos tal cual, y si son varios, todos juntos de una.',
    ...
    parserNote: 'De la Cuenta Corriente (o del histórico por mail) importamos compras, ventas, depósitos, retiros, el interés de cauciones, el resultado de tus futuros de dólar y los dividendos (las conversiones cable↔MEP se omiten). Si el histórico que te mandan trae las columnas Dolares y Mda, separamos solo tu cuenta en pesos de tu cuenta en dólares: lo que se movió en dólares entra en dólares, no al equivalente en pesos que el archivo muestra al lado. Y cuando comprás o vendés dólar MEP (compra de un bono en pesos + venta contra dólares) lo cargamos como UNA conversión, así no te cuenta como plata nueva que pusiste. Como la Cuenta Corriente solo cubre el período del export, la Tenencia valorizada (PDF) es tu foto de HOY y MANDA: completa las posiciones que ya tenías de antes y ajusta lo que quedó de más o de menos (cerrando a costo, sin inventar ganancias). Por seguridad, si tocaría más de la mitad de tu cartera lo frenamos. ¿Ya importaste antes sin la Tenencia? Volvé a importar incluyendo el PDF. Si tenés un fondo común (FCI) abierto, cargalo a mano desde Posiciones.',
  },
  {
    id: 'iol',
    label: 'IOL',
